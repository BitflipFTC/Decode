...
class AutoPoses(
    ...
    private fun p (p: Pose): Pose = if (alliance == Alliance.RED) p else p.mirror().withX(144.0-p.x)

    private val HORIZONTAL_INTAKE_START = 100.0
    private val HORIZONTAL_INTAKE_END = 136.7
    private val HORIZONTAL_INTAKE_MOVE_BACK = 110.0

    private val INTAKE_1_VERTICAL = 84.0
    private val INTAKE_2_VERTICAL = 59.5
    private val INTAKE_3_VERTICAL = 36.0
    private val INTAKE_HEADING = 0.0

    val farStartPose   = p(Pose(83.0, 10.0, Math.toRadians(90.0)))
    val farShootPoseFacingObelisk = p(Pose(84.5, 18.0, Math.toRadians(90.0)))
    val farShootPose   = p(Pose(83.5, 21.0, Math.toRadians(90.0)))
    val farParkPose    = p(Pose(105.500, 33.500, Math.toRadians(90.0)))
    val nearStartPose  = p(Pose(124.5, 116.0, 0.0))
    ...
    val nearShootPoseInZone = p(Pose(88.0, 118.0, Math.toRadians(45.0)))
    val nearParkPose   = p(Pose(120.0, 70.0, Math.toRadians(90.0)))
    ...
    val endIntake1     = p(Pose(128.5, INTAKE_1_VERTICAL, INTAKE_HEADING))
    ...
    val emptyRampStart = p(Pose(124.0, 68.0, Math.toRadians(90.0)))
    val emptyRamp      = p(Pose(130.0, 68.0, Math.toRadians(90.0)))
    val emptyRampControl = p(Pose(nearShootPose.x, INTAKE_2_VERTICAL))
    val redPark        = p(Pose(38.75, 33.25, Math.toRadians(90.0)))
    val HPIntakeEnd    = p(Pose(HORIZONTAL_INTAKE_END, 13.0, Math.toRadians(10.0)))
    val AboveHPIntakeEnd = p(Pose(HORIZONTAL_INTAKE_END, 26.0, Math.toRadians(10.0)))
    val AboveHPIntakeStart = p(Pose(HORIZONTAL_INTAKE_START, 26.0, Math.toRadians(10.0)))
    val HPIntakeEndUp = p(Pose(HORIZONTAL_INTAKE_END, 14.0, 0.0))
    val HPIntakeStart = p(Pose(120.0, 11.0, 0.0))
    val HPPark = p(Pose(120.0, 14.0, 0.0))
    ...
    val farShootTeleopHP = p(Pose(83.0, 9.0, Math.PI))

    val relo = p(Pose(126.0, 83.5, Math.toRadians(90.0)))
}